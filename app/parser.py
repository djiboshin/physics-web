...
def get_seminars(week_forward: int or None = 0):
    ...
    for div in sem_divs:
        ...
        if seminar.type in color_table.keys():
            seminar.color = color_table[seminar.type]
...
def html_to_png(html: str):
    ...
    a = imgkit.from_string(html, output_path=False, options=options)
    inp_b = io.BytesIO(a)
    return inp_b
...
def html_to_pdf(html: str):
    ...
    a = pdfkit.from_string(html, output_path=False, options=options)
    inp_b = io.BytesIO(a)
    return inp_b
